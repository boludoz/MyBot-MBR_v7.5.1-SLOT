...
Func lblTotalWarTroopCount()
    Local $TotalTroopsToTrain
    Local $ArmyCampTemp = 0

    If GUICtrlRead($g_hChkTotalCampForced) = $GUI_CHECKED Then
        $ArmyCampTemp = GUICtrlRead($g_hTxtTotalCampForced)
    Else
        $ArmyCampTemp = $g_iTotalCampSpace
    EndIf
    ...
    If $TotalTroopsToTrain = $ArmyCampTemp Then
        GUICtrlSetBkColor($g_hLblCountWarTroopsTotal, $COLOR_MONEYGREEN)
    ElseIf $TotalTroopsToTrain > $ArmyCampTemp / 2 And $TotalTroopsToTrain < $ArmyCampTemp Then
        GUICtrlSetBkColor($g_hLblCountWarTroopsTotal, $COLOR_ORANGE)
    Else
        GUICtrlSetBkColor($g_hLblCountWarTroopsTotal, $COLOR_RED)
    EndIf

    Local $fPctOfCalculated = Floor(($TotalTroopsToTrain / $ArmyCampTemp) * 100)
...
Func lblTotalWarSpellCount()

    Local $TotalSpellsToBrew
    Local $ArmyCampTemp = 0

    If GUICtrlRead($g_hChkTotalCampForced) = $GUI_CHECKED Then
        $ArmyCampTemp = GUICtrlRead($g_hTxtTotalCountSpell)
    Else
        $ArmyCampTemp = $g_iTotalSpellValue
    EndIf
    ...
    If $TotalSpellsToBrew = $ArmyCampTemp Then
        GUICtrlSetBkColor($g_hLblCountWarSpellsTotal, $COLOR_MONEYGREEN)
    ElseIf $TotalSpellsToBrew > $ArmyCampTemp / 2 And $TotalSpellsToBrew < $ArmyCampTemp Then
        GUICtrlSetBkColor($g_hLblCountWarSpellsTotal, $COLOR_ORANGE)
    Else
        GUICtrlSetBkColor($g_hLblCountWarSpellsTotal, $COLOR_RED)
    EndIf

    Local $fPctOfCalculated = Floor(($TotalSpellsToBrew / $ArmyCampTemp) * 100)
